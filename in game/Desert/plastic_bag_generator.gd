...
@export var spawn_interval = 4
@export var spawn_area = Vector2(1, 556)
@export var projectile_direction = Vector2(-1, 0) # Dirección predeterminada
...
func _process(delta):
	if spawn_interval > 0:
		...
		if spawn_interval <= 0:
			spawn_interval = 1.0  # Restablece el intervalo de generación
			_spawn_projectile()
